...
namespace UnityMCP.Editor.Settings
{
    ...
    internal sealed class McpSettingsProvider : SettingsProvider
    {
        ...
        private void EnsureStyles()
        {
            ...
            {
                this.headerStyle = TryCreateStyle(null, 14, new RectOffset(0, 0, 10, 5));
            }

            if (this.subHeaderStyle == null)
            {
                this.subHeaderStyle = TryCreateStyle(null, 12, new RectOffset(0, 0, 5, 3));
            }

            if (this.descriptionStyle == null)
            {
                this.descriptionStyle = TryCreateStyle(null, null, null);
                this.descriptionStyle.wordWrap = true;
            }
        }

        private void EnsureGuiState()
        {
            if (this.enabledIcon == null)
            {
                this.enabledIcon = EditorGUIUtility.IconContent("TestPassed") ?? new GUIContent("On");
            }

            if (this.disabledIcon == null)
            {
                this.disabledIcon = EditorGUIUtility.IconContent("TestFailed") ?? new GUIContent("Off");
            }

            this.defaultBackgroundColor = GUI.backgroundColor;
        }

        private static GUIStyle TryCreateStyle(GUIStyle source, int? fontSize, RectOffset margin)
        {
            var style = source != null ? new GUIStyle(source) : new GUIStyle();

...
namespace UnityMCP.Editor.Core
{
    ...
    internal sealed class McpHttpServer : IDisposable
    {
        ...
        private int StartHttpListener(int startPort)
        {
            ...
            for (var port = startPort; port <= maxPort; port++)
            {
                try
                {
                    ...
                    // Binding both 127.0.0.1 and localhost can fail on some Windows setups
                    // when they resolve to the same local endpoint. This server is local-only,
                    // so a single loopback prefix is enough.
                    listener.Start();
            ...
            Func<JObject, JObject> handler)
        {
            ...
            // Large projects can spend more than 10s traversing logs, scenes, or compiling Roslyn code.
            // Keep the server responsive, but give main-thread work a wider budget before timing out.
